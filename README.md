<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/версия-v1.5.79-brightgreen?style=flat-square" alt="version">
...
|----------|-------------|
| **VLESS + Reality** | TLS 1.3 masquerade, x25519, shortId, fingerprint (chrome/firefox/safari/random); собственный TLS 1.3 стек на wolfCrypt (wolfSSL не поддерживает x25519 static ephemeral для Reality ECDH) |
| **VLESS + XHTTP** | HTTP/2 (ALPN=h2), Content-Type=application/grpc, session ID в URL path, stream-one (Reality) и stream-up (двунаправленный) режимы |
| **VLESS + gRPC** | HTTP/2 + HPACK + LPM + protobuf framing + flow control, async state machine на 5 состояний без goto |
| **VLESS + WebSocket** | TLS + HTTP Upgrade handshake, MASK=1, switch/case recv state machine |
| **VLESS + HTTPUpgrade** | TLS + HTTP GET без Sec-WebSocket-Key → 101 → raw TCP (без WS framing overhead) |
| **VLESS + TCP plain** | Туннельный HC через VLESS request на `www.gstatic.com:443` (как mihomo url-test) |
| **Trojan / Trojan+gRPC** | Маскировка под HTTPS, мультиплексирование; gRPC variant: TLS+H2+Trojan header через DATA frame |
...
| **Hysteria2** | QUIC-based, Brutal CC, Salamander XOR обфускация, URI парсер; async RELAY_HY2_CONNECT |
...
Два уровня адаптации работают параллельно:

**1. DeviceProfile (compile-time, по `MemTotal`)** — выбирается при сборке через `PROFILE=micro|normal|full` или автодетектом при старте; задаёт verдикт-маги, фейк-IP пул и максимум соединений:

| Профиль | RAM | Fake-IP пул | DNS кэш default | Соединения |
| --- | --- | --- | --- | --- |
| MICRO | < 48 МБ | 512 записей | 128 | 256 |
| NORMAL | < 192 МБ | 4 096 записей | 512 | 1 024 |
| FULL | ≥ 192 МБ | 65 536 записей | 2 048 | 4 096 |

**2. mem_tier (runtime, по `MemAvailable`)** — читается **один раз** при старте из `/proc/meminfo` и выставляет глобальные runtime-лимиты. Один бинарник оптимален и для EC330 (128 МБ), и для Flint 2 (512 МБ+):

| Tier | MemAvailable | dispatcher_max_events | relay_drain_per_call | dns_cache | geo mmap |
| --- | --- | --- | --- | --- | --- |
| LOW | < 64 МБ | 8 | 4 | 512 | MAP_NORESERVE |
| MID | 64–256 МБ | 32 | 16 | 2 048 | MAP_NORESERVE |
| HIGH | > 256 МБ | 64 | 32 | 8 192 | MAP_POPULATE |

`mem_tier` дополняет DeviceProfile: первый отвечает за **ёмкость структур** (compile-time, не зависит от текущей нагрузки), второй — за **скорость на горячем пути** в момент старта (event loop размер, drain batch, mmap preload).
...
- **`4eburnet_VERSION_<arch>.ipk`** — демон + LuCI + dashboard (per-arch: mipsel/aarch64/x86_64).
- **`4eburnet-geo_VERSION_all.ipk`** — pre-built `.gbin` базы (geoip-ru, geosite-ru/ads/trackers/threats, opencck-domains). `arch=all`: один пакет для всех устройств. Обновляется независимо от демона; либо через cdn_updater (скачивает новые версии `.gbin` с GitHub Releases, atomic rename + SIGHUP без рестарта) либо переустановкой пакета.

```sh
# Копировать на роутер
scp 4eburnet_*.ipk 4eburnet-geo_*.ipk root@192.168.1.1:/tmp/

# Установить (порядок: сначала geo, потом демон — geo базы сразу доступны при старте)
