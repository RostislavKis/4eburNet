                     ...
                     size_t secret_len)
{
    ...
    if (write_secret) {
        if (hkdf_expand_label(ssl, write_secret, secret_len,
                               "quic key", key, key_len)    < 0) goto fail;
        if (hkdf_expand_label(ssl, write_secret, secret_len,
                               "quic iv",  iv,  QUIC_IV_LEN) < 0) goto fail;
        if (hkdf_expand_label(ssl, write_secret, secret_len,
                               "quic hp",  hp,  key_len)    < 0) goto fail;
        if (init_aead_ctx(&keys->send_aead, cipher, key, iv, 1) < 0) goto fail;
        if (init_hp_ctx  (&keys->send_hp,   hp, key_len)        < 0) goto fail;
    ...
    if (read_secret) {
        ...
        if (hkdf_expand_label(ssl, read_secret, secret_len,
                               "quic hp",  hp,  key_len)    < 0) goto fail;
        if (init_aead_ctx(&keys->recv_aead, cipher, key, iv, 0) < 0) goto fail;
        if (init_hp_ctx  (&keys->recv_hp,   hp, key_len)        < 0) goto fail;
        memset(key, 0, sizeof(key));
        memset(iv,  0, sizeof(iv));
        memset(hp,  0, sizeof(hp));
    }
...
void quic_keys_free(quic_keys_t *keys)
{
    ...
    /* освобождаем AES-ECB контексты HP по key_len — не только при keys->ready,
     * чтобы корректно работать при частичной инициализации через quic_hp_init() */
    if (keys->send_hp.key_len > 0) {
        wc_AesFree(&keys->send_hp.aes_ecb);
        keys->send_hp.key_len = 0;
    }
    if (keys->recv_hp.key_len > 0) {
        wc_AesFree(&keys->recv_hp.aes_ecb);
        keys->recv_hp.key_len = 0;
    }
    keys->ready = 0;
}
