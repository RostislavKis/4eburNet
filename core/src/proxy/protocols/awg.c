             ...
             int tai_utc_offset)
{
    ...
    /* CPS init packets (указатели на ServerConfig.awg_i[], не копируем) */
                        ...
                        const char *server_ip, uint16_t server_port)
{
    ...
    int cps_sent = 0;
    for (int i = 0; i < 5; i++) {
        const char *cps_str = awg->cfg.i[i];
        if (!cps_str || !cps_str[0]) continue;
        uint8_t cpkt[512];
        size_t clen = sizeof(cpkt);
        awg_cps_build(cps_str, cpkt, &clen);
        if (clen > 0) {
