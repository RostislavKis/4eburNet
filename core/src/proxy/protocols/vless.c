...
/* vless_read_response / vless_handshake — удалены (B-15).
 * Блокирующие legacy-функции с select() на 5 сек внутри event loop.
 * Замена: vless_handshake_start() + vless_read_response_step() из dispatcher.
 * Никем не вызывались — мёртвый код с __attribute__((deprecated)). */
