...
static int tls_handshake_step(async_dns_conn_t *conn)
{
    if (!conn->tls_init) {
        ...
        if (conn->sni[0])
            snprintf(tls_cfg.sni, sizeof(tls_cfg.sni), "%s", conn->sni);
                        ...
                        async_dns_cb_t callback, void *cb_ctx)
{
    ...
    snprintf(conn->sni, sizeof(conn->sni), "%s", host);
    ...
    conn->http_req_len = (size_t)snprintf(
        conn->http_req, sizeof(conn->http_req),
