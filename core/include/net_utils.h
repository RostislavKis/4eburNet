...
/*
 * UDP ping: fork → UDP connect + send 1 байт → poll ответ/ICMP.
 * Формат результата аналогичен TCP: "OK <ms>\n" или "ERR\n".
 * Таймаут = host unreachable/filtered → ERR.
 * ICMP port-unreachable = host UP (порт слушает WG/QUIC, ICMP — от ОС).
 */
