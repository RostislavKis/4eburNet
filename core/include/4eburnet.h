...
#define EBURNET_VERSION "1.5.132"  /* fallback; Makefile.dev передаёт через -D */
#endif
...
/* WHY 65536 (было 16384): YouTube CDN чанки 5-10MB, при 16KB буфере = 600+
 * системных вызовов recv/write per chunk. С 64KB буфером — 4x меньше
 * iter в epoll loop, лучше throughput. RAM impact: один shared buffer
 * на dispatcher = +48KB total, не критично для NORMAL (116MB). */
#define NORMAL_BUFFER_SIZE      65536
#define NORMAL_MAX_RULES        2048
#define NORMAL_DNS_CACHE_SIZE   4096

#define FULL_MAX_CONNECTIONS    16384
#define FULL_BUFFER_SIZE        131072  /* 128KB — для FULL (>128MB RAM) */
#define FULL_MAX_RULES          16384
#define FULL_DNS_CACHE_SIZE     32768

/* DPI значения по умолчанию */
#define EBURNET_DPI_DEFAULT_FAKE_SNI  "www.google.com"
...
#define EBURNET_DPI_DIR         "/etc/4eburnet/dpi"
#define EBURNET_GEO_DIR         "/etc/4eburnet/geo"
...
typedef struct {
    ...
    time_t               cdn_next_check; /* unix timestamp следующей проверки CDN (0 = не инициализирован) */
    int                  ipc_fd;
    time_t               start_time;
...
/* Хук для WS /logs — вызывается после записи каждой строки в файл.
 * Аргумент: уже отформатированная строка без \n.
 * Передать NULL для снятия хука. */
void log_set_hook(void (*hook)(const char *line));
