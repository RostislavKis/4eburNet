...
4eburNet использует **TPROXY** для перехвата трафика на уровне ядра через nftables.
Прозрачно для всех приложений, работает для TCP/UDP трафика всех устройств сети.

Доступные режимы (`option mode` в UCI):
- `rules` — маршрутизация по правилам (рекомендуется)
- `global` — весь трафик через прокси
- `direct` — без прокси (bypass)

> **Примечание:** TUN режим не нужен. TPROXY на роутере перехватывает на уровне ядра
> без двойного копирования через userspace, прозрачно для всех устройств сети.
