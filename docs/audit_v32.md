...
## Post-audit fixes (коммит e0a0a67)

| ID | Действие | Статус |
|----|---------|--------|
| P1 | geo_compile CIDR инкремент только при успешном parse | ✓ |
| P2 | geo_update.sh if-fi вместо `A&&B\|\|C` | ✓ |
| P3 | opencck .gbin пересобирается при автообновлении | ✓ |
| Z1 | strtol + диапазон вместо atoi | ✓ |
| Z3 | тест: невалидный magic → load fail | ✓ |
| Z4 | тест: truncated файл → load fail | ✓ |
| Z5 | тест: cat_type из hdr проверяется | ✓ |
