...
> **СТАТУС: ЗАКРЫТА (волна 12).** pkt/response/tcp_reply на heap + goto cleanup.

### M-02 [dns/dns_server.c:332-333] Upstream ответ без валидации DNS формата
Нет проверки QR=1, resp_n >= 12, question section match.

> **СТАТУС: ЗАКРЫТА (волна 12).** Проверка resp_n < 12 и QR bit (resp[2] & 0x80).
...
> **СТАТУС: ЗАКРЫТА (волна 12).** Tombstone eviction: освобождение LRU tail, re-probe для нового hash.

### M-04 [proxy/trojan.c:39-51] SHA224 hash и context не zeroed после use

> **СТАТУС: ЗАКРЫТА (волна 12).** wc_Sha224Free + explicit_bzero(hash).

### M-05 [proxy/dispatcher.c:838-839] Use-after-free риск при двух events на одном relay

> **СТАТУС: ЗАКРЫТА (волна 12).** Guard: if (r->state == RELAY_DONE) continue.

### M-06 [proxy/dispatcher.c:610-611] SS relay upstream→client: partial write теряет данные

> **СТАТУС: ЗАКРЫТА (волна 12).** Partial write = fatal return -1 для framed SS.

### M-07 [proxy/dispatcher.c:1149-1155] XHTTP download→client: partial write теряет данные

> **СТАТУС: ЗАКРЫТА (волна 12).** Partial write = relay_free для chunked stream.

### M-08 [proxy/dispatcher.c:1216-1224] AWG downstream→client: partial write теряет данные

> **СТАТУС: ЗАКРЫТА (волна 12).** Partial write = relay_free для UDP framing.

### M-09 [crypto/noise.c:502-503] time_t вычитание может wrap на 32-bit mipsel

> **СТАТУС: ЗАКРЫТА (волна 12).** Защита: elapsed = (now >= handshake_time) ? diff : 0.

### M-10 [crypto/tls.c:191-193] malloc failure для reality_key → silent NULL

> **СТАТУС: ЗАКРЫТА (волна 12).** malloc check + tls_close + return -1.

### M-11 [crypto/tls.c:377] reality_key free без zeroing key material

> **СТАТУС: ЗАКРЫТА (волна 12).** explicit_bzero перед free для reality_key и reality_short_id.
...
> **СТАТУС: ЗАКРЫТА (волна 12).** explicit_bzero(buf) в noise_hkdf2/hkdf3.

### L-02 [crypto/blake2s.c:38-67] blake2s_compress: m[16]/v[16] на стеке не zeroed

> **СТАТУС: ЗАКРЫТА (волна 12).** explicit_bzero(m) + explicit_bzero(v).

### L-03 [crypto/noise.c:351] TAI64N +37 устареет при новых leap seconds

> **СТАТУС: ЗАКРЫТА (волна 13).** tai_utc_offset в PhoenixConfig, парсинг из UCI, передача через noise_init. Обновляется без перекомпиляции.

### L-04 [proxy/awg.c:56-60] random_u32 fallback предсказуем

> **СТАТУС: ЗАКРЫТА (волна 12).** xorshift64 fallback вместо LCG.

### L-05 [proxy/dispatcher.c:761-769] AWG client_fd не добавлен в epoll

> **СТАТУС: ЗАКРЫТА (волна 12).** epoll_ctl ADD client_fd в AWG branch.

### L-06 [dns/dns_resolver.c:63-64] Upstream сокет только IPv4

> **СТАТУС: ЗАКРЫТА (волна 12).** AF_INET6 + sockaddr_in6 при наличии ':' в upstream_ip.

### L-07 [dns/dns_resolver.c:108] Timeout granularity: time(NULL) ±1 сек

> **СТАТУС: ЗАКРЫТА (волна 13).** CLOCK_MONOTONIC в dns_pending_t, гранулярность ~1ms, проверка каждый тик (10ms).

### L-08 [dns/dns_rules.c:80-88] Unbounded realloc growth (нет upper cap)

> **СТАТУС: ЗАКРЫТА (волна 12).** DNS_RULES_MAX = 500000.

### L-09 [src/main.c:319] PATH_MAX (2×4096) на стеке main()

> **СТАТУС: ЗАКРЫТА (волна 12).** malloc/free вместо стека.

### L-10 [routing/device_policy.c:257] mac_str не json_escape'd

> **СТАТУС: ЗАКРЫТА (волна 12).** json_escape(mac_str) в device_policy_to_json.

### L-11 [src/config.c:220] config fopen без O_CLOEXEC

> **СТАТУС: ЗАКРЫТА (волна 12).** open(O_CLOEXEC) + fdopen.

### L-12 [Makefile] dns_resolver.c не в production Makefile

> **СТАТУС: ЗАКРЫТА (волна 12).** Добавлен в PHOENIX_SOURCES.
...
| Категория | Найдено | Закрыто | Принято (INFO) |
|-----------|---------|---------|----------------|
| CRITICAL  |    4    |    4    |       0        |
| HIGH      |   14    |   14    |       0        |
| MEDIUM    |   18    |   18    |       0        |
