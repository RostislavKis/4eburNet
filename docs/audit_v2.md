...
✅ **ЗАКРЫТА**: SOCK_CLOEXEC добавлен в socket() для серверного IPC сокета.

**Рекомендация**: `socket(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0)`.

---

### H-02 [ipc.c:155] Клиентски�� IPC сокет без O_CLOEXEC

✅ **ЗАКРЫТА**: SOCK_CLOEXEC добавл��н в socket() для клиентского IPC сокета.

---

### H-03 [dns_upstream.c:27,74] DNS upstream сокеты без O_CLOEXEC

✅ **ЗАКРЫТА**: SOCK_CLOEXEC добавлен во все socket() в dns_upstream.c (UDP, DoT, DoH).
...
✅ **ЗАКРЫТА**: SOCK_CLOEXEC добавлен (покрыто H-03).

---

### H-05 [ntp_bootstrap.c:75] NTP bootstrap сокет без O_CLOEXEC

✅ **ЗАКРЫТА**: SOCK_CLOEXEC добавлен в socket() для NTP bootstrap.

---

### H-06 [tls.c:159] CTX кэш: verify_cert передаётся, но игнорируется при cache hit

✅ **ЗАКРЫТА**: Ключ кэша idx = fp*2 + verify_cert, массив увеличен до 8, bounds check добавлен.
...
✅ **ЗАКРЫТА**: SOCK_NONBLOCK убран из accept4, добавлен SO_RCVTIMEO/SO_SNDTIMEO 2 сек. MSG_WAITALL корректен на blocking сокете с таймаутом.

---

### H-08 [awg.c:34,232] AWG random_fill() и udp_fd без O_CLOEXEC для urandom

✅ **ЗАКРЫТА**: random_fill переписан: getrandom() syscall + O_CLOEXEC fallback + read loop + возврат int с проверкой ошибок.

---

### H-09 [awg.c:234] AWG UDP сокет только IPv4

✅ **ЗАКРЫТА**: awg_handshake_start переписан с sockaddr_storage, поддержка AF_INET и AF_INET6.
...
✅ **ЗАКРЫТА**: Добавлена json_escape() для экранирования спецсимволов. Все строковые поля (name, alias, server_group, comment) экранируются перед вставкой в JSON.
...
✅ **ЗАКРЫТА**: servers, dns_rules, devices_tmp перенесены на heap через calloc. Все return path через goto cleanup_fail с free().

---

### H-12 [main.c:240-241] PhoenixConfig cfg на стеке, указатель в state

⚠️ **ЧАСТИЧНО**: cfg на стеке корректна пока живёт до конца main(). Перенос на malloc -- v2 рефакторинг.
...
✅ **ЗАКРЫТА**: Весь мёртвый код splice удалён: splice path в relay_transfer, has_splice/splice_pipe из структуры и cleanup. O(n) таймаут-скан — ранний выход по checked < conns_count подтверждён.
