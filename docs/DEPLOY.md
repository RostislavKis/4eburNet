...
```powershell
# 1. Собрать в WSL
wsl bash -c "rsync -a --delete /mnt/d/Проекты/4eburNet/ ~/4eburnet-dev/project/4eburNet/"
wsl bash -c "cd ~/4eburnet-dev/project/4eburNet/core && make clean -f Makefile.dev && make -f Makefile.dev cross-mipsel"

# 2. Скопировать собранный бинарник из WSL в Windows
wsl bash -c "cp ~/4eburnet-dev/project/4eburNet/prebuilt/mipsel/4eburnetd /mnt/d/Проекты/4eburNet/prebuilt/mipsel/4eburnetd"

# 3. Остановить демон и удалить старый бинарник (иначе: Text file busy)
ssh root@192.168.2.1 "killall 4eburnetd 2>/dev/null; sleep 2; rm -f /usr/sbin/4eburnetd"

# 4. Скопировать новый бинарник на роутер (-O legacy SCP, OpenWrt без sftp-server)
...
| prebuilt/mipsel в Windows — старый | WSL build пишет в ~/..., не в /mnt/d/ | `cp ~/4eburnet-dev/.../prebuilt/mipsel/4eburnetd /mnt/d/Проекты/.../prebuilt/mipsel/4eburnetd` |
